...
**Prevention**: Repeat the same spatial anchor sentence verbatim in every shot
of a scene ("Character A on screen left facing right, Character B on screen
right facing left"). Generate one wide establishing shot first and reuse its
keyframe as a `reference` or `first_frame` input for tighter shots — pixel lock
beats text description for spatial consistency.
